...
function loaddata() {
	...
	request = $.getJSON('https://api.spotify.com/v1/tracks/?ids='+songarray, function(data){
		...
		d_start = new Date()
		d_end = new Date(d_start.getTime()+15000)
		gameloop()
		interval  = setInterval(gameloop, 10000);
		animation = setInterval(animation, 30);
...
function animation(){
	...
	percent = Math.round((d_end.getTime() - cur_time.getTime())/ 150).toString()
	console.log(percent+"%")
...
function buildSongArrayQuery(songarray) {
